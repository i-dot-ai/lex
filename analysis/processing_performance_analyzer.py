...
class ProcessingPerformanceAnalyzer(BaseAnalyzer):
    ...
    def get_processing_throughput(self) -> Dict[str, any]:
        ...
        query = {"query": {"bool": {"must": [{"match": {"message": "Uploaded batch"}}]}}}
        ...
        # Find peak hour
        peak_hour = (
            max(hourly_stats.items(), key=lambda x: x[1]["documents"])
            if hourly_stats
            else (None, {"documents": 0})
        )
    ...
    def get_processing_time_by_type(self) -> Dict[str, Dict]:
        ...
        doc_type_stats = defaultdict(
            lambda: {"parse_times": [], "scrape_times": [], "total_documents": 0}
        )
    ...
    def get_batch_size_analysis(self) -> Dict[str, any]:
        ...
        query = {"query": {"match": {"message": "batch of"}}}
        ...
        if batch_sizes:
            return {
                ...
                "common_batch_sizes": dict(
                    sorted(batch_size_distribution.items(), key=lambda x: x[1], reverse=True)[:5]
                ),
            }
        else:
            return {
    ...
    def get_memory_usage_patterns(self) -> Dict[str, any]:
        ...
        for log in logs:
            ...
            memory_events.append({"timestamp": timestamp, "message": message[:200], "level": level})
    ...
    def print_report(self):
        ...
        print(
            f"Peak hour: {throughput['peak_hour']} ({throughput['peak_hour_documents']:,} documents)"
        )
        ...
        print(
            f"Batch size range: {batch_analysis['min_batch_size']} - {batch_analysis['max_batch_size']}"
        )

...
class XMLCompletenessComprehensiveAnalyzer(BaseAnalyzer):
    ...
    def get_xml_completeness_aggregated(self, doc_type: Optional[str] = None) -> Tuple[Dict, Dict]:
        ...
        try:
            ...
            success_data = {}
            for year_bucket in success_response.get("aggregations", {}).get("by_year", {}).get("buckets", []):
            ...
            failure_data = {}
            for year_bucket in failure_response.get("aggregations", {}).get("by_year", {}).get("buckets", []):
                year = year_bucket.get("key")
                if year and year.isdigit():
                    year_int = int(year)
        ...
        if all_years:
            ...
            if modern_years:
                ...
                if modern_total > 0:
                    ...
                    results["insights"].append(
                        f"Modern era (2000+): {modern_rate:.1f}% digitized"
                    )
            ...
            if historical_years:
                ...
                if hist_total > 0:
                    ...
                    results["insights"].append(
                        f"Historical (pre-2000): {hist_rate:.1f}% digitized"
                    )
    ...
    def print_report(self, specific_type: Optional[str] = None):
        ...
        if specific_type:
            ...
            print(f"Total documents: {results['summary']['total_xml'] + results['summary']['total_pdf']:,}")
            print(f"XML available: {results['summary']['total_xml']:,}")
            print(f"PDF only: {results['summary']['total_pdf']:,}")
            print(f"Overall digitization rate: {results['summary']['overall_digitization_rate']:.1f}%")
            ...
            for year in sorted(results["by_year"].keys()):
                ...
                print(f"{year:<6} {data['xml_count']:<10} {data['pdf_count']:<10} "
            ...
            for leg_type in sorted(by_type.keys(), key=lambda x: by_type[x]["total"], reverse=True)[:20]:
                data = by_type[leg_type]
                if data["total"] > 0:
                    xml_pct = 100 * data["xml_success"] / data["total"]
                    print(f"{leg_type:<10} {data['total']:<12,} {data['xml_success']:<12,} "

...
class ErrorTypeAnalyzer(BaseAnalyzer):
    ...
    def get_errors_by_time(self, time_bucket: str = "1h") -> Dict[str, List[Dict]]:
        ...
        for bucket in result.get("errors_over_time", {}).get("buckets", []):
            time_series.append(
                {"timestamp": bucket["key_as_string"], "error_count": bucket["doc_count"]}
            )
    ...
    def get_top_error_messages(self, limit: int = 20) -> List[Dict[str, any]]:
        ...
        # Sort by frequency
        top_errors = []
        for normalized_msg, instances in sorted(
    ...
    def get_error_rate_by_document_type(self, hours_back=None) -> Dict[str, Dict[str, float]]:
        ...
        success_query = {
            "query": {
                "bool": {"must": [{"term": {"level": "INFO"}}, {"match": {"message": "Parsed"}}]}
            }
        }
        ...
        error_query = {
            "query": {
                "bool": {
                    "must": [{"term": {"level": "ERROR"}}, {"match": {"message": "Error parsing"}}]
                }
            }
        }
        ...
        # Add time filter if specified
        if hours_back:
            time_filter = {"filter": {"range": {"timestamp": {"gte": f"now-{hours_back}h"}}}}
    ...
    def analyze_non_pdf_errors(self, hours_back=24):
        ...
        validation_query = {
            "query": {
                "bool": {
                    "must": [{"match": {"message": "validation error"}}],
                    "must_not": [{"match_phrase": {"message": "no body found"}}],
                    "filter": {"range": {"timestamp": {"gte": f"now-{hours_back}h"}}},
                }
            },
            "size": 200,
        ...
        if validation_errors:
            ...
            for pattern, errs in patterns.items():
                if errs:
                    ...
                    for err in errs[:2]:
                        print(
                            f"    - {err.get('timestamp', 'N/A')}: {err.get('message', '')[:100]}..."
                        )
    ...
    def print_report(self):
        ...
        for doc_type, stats in sorted(doc_type_errors.items()):
            print(
                f"{doc_type:<20} {stats['total_attempts']:<12,} "

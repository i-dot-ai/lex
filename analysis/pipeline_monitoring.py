...
class PipelineMonitor(BaseAnalyzer):
    ...
    def check_pipeline_status(self, hours_back=None):
        ...
        completion_query = {
            "query": {
                "bool": {"must": [{"match_phrase": {"message": "Pipeline processing complete"}}]}
            },
            "size": 10,
    ...
    def _check_active_pipelines(self, hours_back):
        ...
        start_query = {
            "query": {"bool": {"must": [{"match_phrase": {"message": "Pipeline starting"}}]}},
            "size": 10,
            "sort": [{"timestamp": {"order": "desc"}}],
        }
    ...
    def check_structured_logging(self, hours_back=1):
        ...
        # Sample recent logs
        sample_query = {
            "query": {"bool": {"filter": {"range": {"timestamp": {"gte": f"now-{hours_back}h"}}}}},
            "size": 100,
            "sort": [{"timestamp": {"order": "desc"}}],
        }
    ...
    def check_progress_tracking(self, hours_back=None):
        ...
        progress_query = {
            "query": {"bool": {"must": [{"match_phrase": {"message": "Progress update"}}]}},
            "size": 100,
    ...
    def _check_batch_uploads(self, hours_back):
        ...
        batch_query = {
            "query": {"bool": {"must": [{"match_phrase": {"message": "Uploaded batch"}}]}},
            "size": 1000,
        }

...
class LexMonitoring:
        ...
        if self.backend.configure():
            try:
                ...
                self.api_request_counter = self.meter.create_counter(
                    name="lex_api_requests", 
                    description="Number of API endpoint requests",
                    unit="1"
                )
                ...
                self.mcp_session_counter = self.meter.create_counter(
                    name="lex_mcp_sessions",
                    description="MCP client session events",
                    unit="1"
                )
                
                self.logger.info(f"OpenTelemetry monitoring configured successfully with {type(self.backend).__name__}")
    ...
    def track_page_view(self, request: Request, page_name: str):
        ...
        self.logger.info(f"Page view: {page_name}", extra={
            "custom_dimensions": {
    ...
    def track_api_usage(self, request: Request, endpoint: str, duration: float, status_code: int, 
        ...
        with self.tracer.start_as_current_span(f"api_call_{endpoint.replace('/', '_')}") as span:
            ...
            if query_params:
                span.set_attributes({
                    ...
                    "query.has_filters": any(k in query_params for k in ["year_from", "year_to", "court", "types"]),
                    "query.limit": query_params.get("limit", 0) if query_params.get("limit") else 0,
                })
            ...
            self.request_duration.record(duration * 1000, {
                "endpoint": endpoint,
                "method": request.method,
                "status_code": str(status_code)
            })
            
            span.set_status(Status(StatusCode.OK) if status_code < 400 else Status(StatusCode.ERROR))
    
    def track_mcp_event(self, event_type: str, client_info: Dict[str, Any] = None, 
        ...
        with self.tracer.start_as_current_span(f"mcp_{event_type}") as span:
            span.set_attributes({
                ...
                "mcp.client_name": client_info.get("name", "unknown") if client_info else "unknown",
                "mcp.client_version": client_info.get("version", "unknown") if client_info else "unknown", 
                "mcp.protocol_version": client_info.get("protocolVersion", "unknown") if client_info else "unknown",
            ...
            self.mcp_event_counter.add(1, {
                "event_type": event_type,
                "client_name": client_info.get("name", "unknown") if client_info else "unknown",
                "protocol_version": client_info.get("protocolVersion", "unknown") if client_info else "unknown"
            })
            
            span.set_status(Status(StatusCode.OK))
            
        # Structured logging for MCP analytics
        self.logger.info(f"MCP event: {event_type}", extra={
            "custom_dimensions": {
    ...
    def track_mcp_tool_execution(self, tool_name: str, duration: float, success: bool, 
                               client_info: Dict[str, Any] = None, error_type: str = None,
        ...
        # Record metrics
        self.mcp_tool_invocation_counter.add(1, {
            "tool_name": tool_name,
            "success": str(success),
            "client_type": client_info.get("name", "unknown") if client_info else "unknown"
        })
        
        self.mcp_tool_duration.record(duration * 1000, {
            "tool_name": tool_name,
            "success": str(success)
        })
    ...
    def track_mcp_session_lifecycle(self, event: str, session_id: str, client_info: Dict[str, Any]):
        ...
        self.mcp_session_counter.add(1, {
            "event": event,
            "client_name": client_info.get("name", "unknown")
        })
        
        self.logger.info(f"MCP session {event}: {session_id}", extra={
            "custom_dimensions": {
    ...
    def track_rate_limit_event(self, request: Request, limit_type: str, current_count: int, 
                             limit_value: int, exceeded: bool = False):
    ...
    def track_error(self, request: Request, error: Exception, endpoint: str):
        ...
        self.logger.error(f"Application error in {endpoint}: {error}", extra={
            "custom_dimensions": {
                "event_type": "error",
